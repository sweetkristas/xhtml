	...
	std::vector<LayoutBoxPtr> InlineLayoutBox::layoutInlineWidth(const Dimensions& containing, FixedPoint& width)
	{
		...
		if(node != nullptr && node->id() == NodeId::TEXT) {
			// XXX I don't like this dynamic cast.
			...
			if((getDims().border_.left + getDims().padding_.left) + width > containing.content_.width) {
				width = static_cast<int>(getDims().border_.left + getDims().padding_.left * fixed_point_scale);
			}

			tnode->transformText(containing.content_.width >= 0);

			auto it = tnode->begin();
			while(it != tnode->end()) {
				LinePtr line = tnode->reflowText(it, containing.content_.width);
			}
